 ...
 * @var iterable<\App\Model\Entity\Tutor> $tutors
 */
		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Tutor'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
...
<div class="row">
	...
	<div class="table-responsive">
		...
		?>
                <tr>
					<th>#</th>
                    <th><?= $this->Paginator->sort('tutor_name') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                  ...
                  <td>
					<?php 
        if ($tutor->availability== 1) {
            echo '<span class="badge text-bg-success">Available</span>';
        } elseif ($tutor->availability == 0) {
            echo '<span class="badge text-bg-warning">Inavailable</span>';
        } elseif ($tutor->availability == 2) {
            echo '<span class="badge text-bg-primary">Archived</span>';
        } else {
...
if ($tutor->status == 1) {
    ...
    echo '<span class="badge text-bg-warning">Disabled</span>';
} elseif ($tutor->status == 2) {
    echo '<span class="badge text-bg-danger">Archived</span>';
}
?>
</td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $tutor->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $tutor->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Tutors: "{0}"?', $tutor->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_tutors_active; ?></h3>
					<p>Active Tutors</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_tutors_archived; ?></h3>
					<p>Archived Tutors</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Tutors (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Tutors by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Tutors(s)',
			data: [<?= json_encode($total_tutors_active); ?>, <?= json_encode($total_tutors_disabled); ?>, <?= json_encode($total_tutors_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Tutors','action' => 'index']]); ?>
				<fieldset>
